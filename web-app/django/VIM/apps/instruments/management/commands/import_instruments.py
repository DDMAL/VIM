...
class Command(BaseCommand):
    ...
    def parse_instrument_data(self, instrument_id: str, instrument_data: dict) -> dict:
        ...
        ins_names = dict([(value["language"], value["value"]) for key, value in ins_labels.items()])
    ...
    def get_instrument_data(self, instrument_ids: list[str]) -> list[dict]:
        ...
        instrument_data = [
            self.parse_instrument_data(key, value) for key, value in response_entities.items()
        ]
        return instrument_data

    def handle(self, *args, **options):
        with open("startup_data/vim_instruments_with_images-15sept.csv", encoding = 'utf-8-sig') as csvfile:
        ...
        with transaction.atomic():
            for ins_i in range(0, len(instrument_list), 50):
                ins_ids_subset: list[str] = [ins["instrument"].split("/")[-1] for ins in  instrument_list[ins_i:ins_i+50]]
                ins_data: list[dict] = self.get_instrument_data(ins_ids_subset)
                ins_imgs_subset: list[str] = [ins["image"] for ins in  instrument_list[ins_i:ins_i+50]]
